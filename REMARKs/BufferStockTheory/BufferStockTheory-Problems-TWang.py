...
# <p style="text-align: center;">Generated-Source-REMARK-make</p>
...
#
# This notebook uses the [Econ-ARK/HARK](https://github.com/econ-ark/hark) toolkit to describe the main results and reproduce the figures in the paper [Theoretical Foundations of Buffer Stock Saving](http://econ.jhu.edu/people/ccarroll/papers/BufferStockTheory).
#
...
# %% {"code_folding": [0], "deletable": true, "editable": true}
# PROBLEM 
# The "right" way to do this if you have the latest version of HARK (as of 2019/02/03) cloned
# to your computer, is via the command below; but this version includes some fixes that are 
# not in the latest pip installable version of HARK, so we have included the ConsIndShockModel.py
# file directly in the repo
...
# %% {"deletable": true, "editable": true}
m1= np.linspace(0,4,2000)
m2= np.linspace(0,4,1000)
m3= np.linspace(0,4,200)
m4= np.linspace(0,4,100)

m_grid_lst = np.array([m1,m2,m3,m4])

## Limiting case with artifical borrowing constraint

Constrained_case = IndShockConsumerType(**base_params)
Constrained_case.UnempPrb = 0.00
Constrained_case.BoroCnstArt=0.00
Constrained_case.cycles = 100   # Make this type have a finite horizon (Set T = 100)
Constrained_case.updateIncomeProcess()
Constrained_case.solve()        # Solve the model
Constrained_case.unpackcFunc()  # Make the consumption function easily accessible
c_t1 = Constrained_case.cFunc[-2](m1)

## Plot the consumption function in the second to the last period.  
...
plt.plot(m1,c_t1,'k--',label='Borrowing Constrained')

# Different Unemployment Risk

uem_lst = np.linspace(0.0005,0.005,4)

for i in range(len(uem_lst)):
    Diff_Uemprob_example = IndShockConsumerType(**base_params)
    Diff_Uemprob_example.UnempPrb = uem_lst[i]
    Diff_Uemprob_example.BoroCnstArt = None
    #print(Diff_Uemprob_example.UnempPrb)
    Diff_Uemprob_example.T_cycles = 100
    Diff_Uemprob_example.updateIncomeProcess()
    start_time = clock()
    Diff_Uemprob_example.solve()
    end_time = clock()
    #print('Solving a baseline buffer stock saving model (100 periods) took ' + mystr(end_time-start_time) + ' seconds.')
    Diff_Uemprob_example.unpackcFunc()
    Diff_Uemprob_example.timeFwd()
    m = m_grid_lst[i] # Set the plot range of m
    c_m2 = Diff_Uemprob_example.cFunc[-2](m)
    plt.plot(m,c_m2,label='$UemProb={}$'.format(round(uem_lst[i],5)))
plt.legend()
...
# %% {"deletable": true, "editable": true}
## Solutions
#Computing the discount factor such that GIF=1

GIC_Fail_example = IndShockConsumerType(cycles=0,**base_params)
EPermShkInv=np.dot(GIC_Fail_example.PermShkDstn[0][0],1/GIC_Fail_example.PermShkDstn[0][1])
PermGroFacAdj=GIC_Fail_example.PermGroFac[0]*EPermShkInv
beta_GIC_fail=(PermGroFacAdj/GIC_Fail_example.LivPrb[0])**CRRA/Rfree

print('The cut-off value of discount factor such that GIF=1 is '+str(beta_GIC_fail))

# %% {"deletable": true, "editable": true}
## Solutions

nn_grid = 30
DiscFac_list = np.linspace(0.96,beta_GIC_fail-0.005,nn_grid)
nn_grid = len(DiscFac_list)
mNrmSS_list =np.zeros([nn_grid,1])

for i in range(len(DiscFac_list)):
    GIC_Fail_example = IndShockConsumerType(cycles=0,**base_params)
    GIC_Fail_example.DiscFac = DiscFac_list[i]
    GIC_Fail_example.updateIncomeProcess()
    #GIC_Fail_example.checkConditions()
    GIC_Fail_example.solve()
    GIC_Fail_example.unpackcFunc()
    mNrmSS_list[i]=GIC_Fail_example.solution[0].mNrmSS
    
plt.figure(figsize = (12,8))
plt.xlim(0.96,beta_GIC_fail-0.02)
plt.plot(DiscFac_list,mNrmSS_list,color='black')
plt.xlabel('Discount Factor')
plt.ylabel('Targeted Wealth Ratio')

# %% {"deletable": true, "editable": true}
## Solutions 
## A different way to show it

m = np.linspace(0,50,1000)
E_m = mSSfunc(m)
plt.figure(figsize = (12,8))
plt.plot(m,E_m,color="black",label='$\mathrm{\mathsf{E}}_{t}[\Delta m_{t+1}] = 0$')
plt.xlim(0,50)
plt.ylim(0,10)
plt.xlabel('m')
plt.ylabel('c')

DiscFac_list = np.linspace(0.96,1.02,4)
...
for i in range(len(DiscFac_list)):
    ...
    GIC_Fail_example.solve()
    GIC_Fail_example.unpackcFunc()
    c_m = GIC_Fail_example.cFunc[0](m)    
    plt.plot(m,c_m,label=r'$\beta={}$'.format(DiscFac_list[i]))
    print('When discount factor is '+str(round(DiscFac_list[i],3))+', the targeted wealth is '+str(GIC_Fail_example.solution[0].mNrmSS))
plt.legend()

# %% {"code_folding": [], "deletable": true, "hidden": true, "editable": true}
...
# %% [markdown] {"deletable": true, "hidden": true, "editable": true}
...
# %% {"code_folding": [0], "deletable": true, "hidden": true, "editable": true}
...
# %% [markdown] {"code_folding": [], "deletable": true, "editable": true}
...
# %% [markdown] {"deletable": true, "heading_collapsed": true, "editable": true}
...
# %% {"code_folding": [], "editable": true, "deletable": true, "hidden": true}
